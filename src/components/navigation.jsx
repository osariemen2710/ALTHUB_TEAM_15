  ...
  return (
    <>
      ...
      <div className="md:hidden p-4 flex items-center justify-between border-b border-gray-200">
        <img src={Header} alt="Header" className="h-8" />
        ...
        <div className="space-y-2">
          ...
          <div className="relative w-full px-3 mb-4">
            ...
            <input
              type="text"
              placeholder="Search..."
              className="w-full pl-10 pr-4 py-2 text-sm text-gray-600 bg-gray-50 rounded-lg border border-gray-200 focus:outline-none focus:border-green-500 focus:ring-1 focus:ring-green-500"
            />
          </div>
          ...
          {navItems.map((item) => (
            ...
            >
              {React.cloneElement(item.icon, {
                className: `w-5 h-5 ${location.pathname === item.path ? 'text-green-600' : ''}`
              })}
              <span className={`text-sm ${item.isSpecial ? 'font-medium' : ''}`}>
                {item.label}
              </span>
            </div>
          ))}
